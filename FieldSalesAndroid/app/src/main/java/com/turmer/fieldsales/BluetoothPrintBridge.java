...
public class BluetoothPrintBridge {
     ...
     *   1. Secure SPP socket (standard)
     *   2. Insecure SPP socket (most thermal printers accept this)
     *   3. Reflection-based socket on port 1 (last resort)
    ...
    public String connect(String deviceAddress) {
        try {
            if (!checkBluetoothPermission()) {
                return "ERROR: Bluetooth permission not granted. Please restart the app.";
            }
            ...
            if (device == null) {
                return "ERROR: Device not found: " + deviceAddress;
            }

            connectedDeviceName = device.getName() != null ? device.getName() : deviceAddress;

            // Cancel discovery — speeds up connection and reduces interference
            bluetoothAdapter.cancelDiscovery();

            // Try Method 1: Secure SPP socket
            try {
            ...
            // Try Method 2: Insecure SPP socket (works with most thermal printers)
            try {
            ...
            // Try Method 3: Reflection port 1 (last resort for stubborn devices)
            try {
                ...
                safeCloseSocket();
                connectedDeviceName = "";
                return "ERROR: Cannot connect to " + connectedDeviceName +
                    ". Make sure the printer is on and paired. (" + e3.getMessage() + ")";
            }
