...
public class LoginActivity extends Activity {
    ...
    // The full app URL that MainActivity should load after login
    private static final String FIELD_SALES_PATH  = "/web#action=tts_field_sales";
    ...
    private void authenticate(String baseUrl, String username,
        ...
        new Thread(() -> {
            try {
                ...
                // Save base URL (for future auth) — full app URL has FIELD_SALES_PATH appended
                getSharedPreferences(PREFS_NAME, MODE_PRIVATE).edit()
                        .putString(KEY_ODOO_URL, baseUrl).apply();

                // The WebView should open the Field Sales app, not the Odoo home page
                String appUrl = baseUrl + FIELD_SALES_PATH;

                showLoading(false);
                runOnUiThread(() -> {
                    Intent intent = new Intent(LoginActivity.this, MainActivity.class);
                    intent.putExtra("odoo_url",      appUrl);    // Field Sales app URL
