...
public class EscPosPrinter {
    ...
    public String connect() {
        ...
        SharedPreferences prefs = context.getSharedPreferences("FieldSalesPrefs", Context.MODE_PRIVATE);
        String savedMac = prefs.getString("selected_printer_mac", "");

        if (!savedMac.isEmpty()) {
            try {
                BluetoothDevice device = bluetoothAdapter.getRemoteDevice(savedMac);
                Log.d(TAG, "Connecting to selected printer: " + device.getName() + " [" + savedMac + "]");
                socket = device.createRfcommSocketToServiceRecord(SPP_UUID);
                socket.connect();
                outputStream = socket.getOutputStream();
                return "OK";
            } catch (Exception e) {
                Log.e(TAG, "Failed to connect to selected printer: " + savedMac, e);
                close();
                return "Could not connect to selected printer. Is it turned on?";
            }
        }

        // Fallback: If no printer manually selected, try to find a default one
